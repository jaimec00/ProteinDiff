...
class TestEdgeEncoder:
    ...
    def edge_encoder(self, device):
        ...
        return EdgeEncoder(cfg).to(device)

    @pytest.fixture
    def small_edge_encoder(self, device):
    ...
    def sample_protein(self, device):
        ...
        coords_bb = torch.zeros(ZN, 4, 3, device=device)
        for i in range(ZN):
            base_x = i * 3.8
            coords_bb[i, 0] = torch.tensor([base_x, 0.0, 0.0], device=device) + torch.randn(3, device=device) * 0.1  # N
            coords_bb[i, 1] = torch.tensor([base_x + 1.46, 0.0, 0.0], device=device) + torch.randn(3, device=device) * 0.1  # CA
            coords_bb[i, 2] = torch.tensor([base_x + 2.98, 0.0, 0.0], device=device) + torch.randn(3, device=device) * 0.1  # C
            coords_bb[i, 3] = torch.tensor([base_x + 3.21, 1.23, 0.0], device=device) + torch.randn(3, device=device) * 0.1  # O

        # Create orthonormal frames for each residue
        frames = torch.zeros(ZN, 3, 3, device=device)
        for i in range(ZN):
            # Simple rotation around z-axis by small random angle
            angle = torch.randn(1, device=device) * 0.1
            cos_a, sin_a = torch.cos(angle), torch.sin(angle)
            frames[i, 0, :] = torch.tensor([cos_a, sin_a, 0.0], device=device)  # x-axis
            frames[i, 1, :] = torch.tensor([-sin_a, cos_a, 0.0], device=device)  # y-axis
            frames[i, 2, :] = torch.tensor([0.0, 0.0, 1.0], device=device)  # z-axis

        # Create sequence and chain positions (single chain)
        seq_pos = torch.arange(ZN, device=device)
        chain_pos = torch.zeros(ZN, dtype=torch.long, device=device)
    ...
    def test_get_neighbors_batched(self, small_edge_encoder, device):
        """Test neighbor finding with multiple samples in batch."""
        torch.manual_seed(123)

        # Create two separate chains with 3 residues each
        ZN = 6
        coords_bb = torch.randn(ZN, 4, 3, device=device) * 10.0

        # First 3 residues are sample 0, last 3 are sample 1
        sample_idx = torch.tensor([0, 0, 0, 1, 1, 1], device=device)
    ...
    def test_get_frames(self, small_edge_encoder, sample_protein):
        ...
        for i in range(min(3, ZN)):  # Check first few
            for j in range(min(2, K)):
                ...
                assert torch.allclose(RTR, torch.eye(3, device=R.device), atol=1e-4)
    ...
    def test_get_seq_pos_different_chains(self, small_edge_encoder, device):
        ...
        coords_bb = torch.randn(ZN, 4, 3, device=device) * 10.0
        seq_pos = torch.arange(ZN, device=device)
        chain_pos = torch.tensor([0, 0, 0, 1, 1, 1], device=device)  # Two chains
        sample_idx = torch.zeros(ZN, dtype=torch.long, device=device)
    ...
    def test_deterministic_output(self, small_edge_encoder, device):
        ...
        ZN = 5
        coords_bb = torch.randn(ZN, 4, 3, device=device) * 10.0
        frames = torch.eye(3, device=device).unsqueeze(0).expand(ZN, 3, 3).clone()
        seq_pos = torch.arange(ZN, device=device)
        chain_pos = torch.zeros(ZN, dtype=torch.long, device=device)
        sample_idx = torch.zeros(ZN, dtype=torch.long, device=device)
...
class TestMPNNBlock:
    ...
    def mpnn_block(self, device):
        ...
        return MPNNBlock(cfg).to(device)

    @pytest.fixture
    def small_mpnn_block(self, device):
    ...
    def sample_graph(self, device):
        ...
        nodes = torch.randn(ZN, d_model, device=device)
        edges = torch.randn(ZN, K, d_model, device=device)
        nbrs = torch.randint(0, ZN, (ZN, K), device=device)
        nbr_mask = torch.ones(ZN, K, dtype=torch.bool, device=device)
    ...
    def test_mpnn_block_no_edge_update(self, device):
    ...
    def test_masking_effect(self, small_mpnn_block, device):
        ...
        # Run with full mask
        nbr_mask_full = torch.ones(ZN, K, dtype=torch.bool, device=device)
...
class TestMPNNModel:
    ...
    def mpnn_model(self, device):
        ...
        return MPNNModel(cfg).to(device)

    @pytest.fixture
    def small_mpnn_model(self, device):
    ...
    def sample_protein(self, device):
        ...
        for i in range(ZN):
            angle = torch.randn(1, device=device) * 0.1
            cos_a, sin_a = torch.cos(angle), torch.sin(angle)
            frames[i, 0, :] = torch.tensor([cos_a, sin_a, 0.0], device=device)
            frames[i, 1, :] = torch.tensor([-sin_a, cos_a, 0.0], device=device)
            frames[i, 2, :] = torch.tensor([0.0, 0.0, 1.0], device=device)
    ...
    def test_batched_processing(self, small_mpnn_model, device):
        ...
        coords_bb = torch.randn(ZN, 4, 3, device=device) * 10.0
        frames = torch.eye(3, device=device).unsqueeze(0).expand(ZN, 3, 3).clone()
        seq_pos = torch.tensor([0, 1, 2, 0, 1, 2], device=device)
        chain_pos = torch.tensor([0, 0, 0, 1, 1, 1], device=device)
        sample_idx = torch.tensor([0, 0, 0, 1, 1, 1], device=device)
        nodes = torch.randn(ZN, d_model, device=device)
    ...
    def test_multiple_blocks(self, mpnn_model, sample_protein, device):
        """Test that multiple MPNN blocks process sequentially."""
        coords_bb, frames, seq_pos, chain_pos, sample_idx, nodes = sample_protein
        # Need to adjust node dimension to match mpnn_model's d_model=128
        nodes = torch.randn(coords_bb.shape[0], 128, device=device)
    ...
    def test_deterministic_output(self, small_mpnn_model, device):
    ...
    def test_varying_sequence_lengths(self, small_mpnn_model, device):
