...
def anglogram_loss_fwd_bwd(
    ...
    # strides - unit_vecs (ZN, 3)
    stride_vecs_zn, stride_vecs_3,
    ...
    # strides - d_unit_vecs (ZN, 3)
    stride_dvecs_zn, stride_dvecs_3,
    # block sizes
    BLOCK_D: tl.constexpr,
):
    """Fused anglogram loss forward and backward.
    ...
    Uses dot product of unit vectors with fixed range [-1, 1].
    """
    ...
    # load unit_vecs[i]
    vi_x = tl.load(unit_vecs_ptr + pid * stride_vecs_zn + 0)
    vi_y = tl.load(unit_vecs_ptr + pid * stride_vecs_zn + 1)
    vi_z = tl.load(unit_vecs_ptr + pid * stride_vecs_zn + 2)
    ...
    for j in range(seq_start, seq_end):
        ...
        # load unit_vecs[j]
        vj_x = tl.load(unit_vecs_ptr + j * stride_vecs_zn + 0)
        vj_y = tl.load(unit_vecs_ptr + j * stride_vecs_zn + 1)
        vj_z = tl.load(unit_vecs_ptr + j * stride_vecs_zn + 2)

        # compute dot product
        dot = vi_x * vj_x + vi_y * vj_y + vi_z * vj_z

        # bin index (target)
        bin_idx_f = (dot - min_val) / bin_width
...
class AnglogramLoss(torch.autograd.Function):
    ...
    def forward(
        ...
        # assertions
        assert unit_vecs.shape == (ZN, 3), f"unit_vecs {unit_vecs.shape} != ({ZN}, 3)"
        ...
        # unit_vecs in fp32 for dot product computation, logits/fc1/fc2 in fp16 for tensor core
        ...
        d_unit_vecs = torch.zeros(ZN, 3, device=unit_vecs.device, dtype=torch.float32)
        ...
        anglogram_loss_fwd_bwd[grid](
            ...
            # strides - unit_vecs
            unit_vecs.stride(0), unit_vecs.stride(1),
            # strides - d_logits
            ...
            # strides - d_unit_vecs
            d_unit_vecs.stride(0), d_unit_vecs.stride(1),
            # block size
            BLOCK_D,
        )
