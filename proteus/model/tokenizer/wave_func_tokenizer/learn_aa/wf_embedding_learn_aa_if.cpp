...
// declare the cuda kernel implemented in wf_embedding_learn_aa_kernel.cu
...
void wf_embedding_learn_aa_forward(
    ...
    TORCH_CHECK(d_aa.size(0) == tot_BL, "d_aa BL size mismatch");
    TORCH_CHECK(d_aa.size(1) == d_model, "d_aa d_model size mismatch");
    ...
    float* d_aa_ptr = d_aa.data_ptr<float>();

    // launch the cuda kernel
    cudaStream_t stream = at::cuda::getCurrentCUDAStream();
    wf_embedding_learn_aa_kernel_forward(
        coordsA_ptr,
        ...
        d_aa_ptr,
        d_aa.stride(0), d_aa.stride(1), d_aa.stride(2),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &wf_embedding_learn_aa_forward, "Wavefunction Embedding (Learn AA) Forward Method");
}
