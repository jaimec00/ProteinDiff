...
from model.div_vae import DivergenceVAE
from model.bb_vae import BackBoneVAE
...
class ProteinDiff(nn.Module):
	def __init__(self,	d_model: int=256, d_diffusion: int=256, d_latent: int=16, top_k: int=16,
						...
						min_rbf: float=2.0, max_rbf: float=22.0, num_rbf: int=16,
		...
		self._div_vae = DivergenceVAE(voxel_dim=voxel_dims, d_model=d_model, d_latent=d_latent, resnet_layers=vae_layers)
	...
	def forward(self, 	coords: torch.Tensor, labels: torch.Tensor, 
		...
		match run_type:
			...
			case "div_vae": return self._run_div_vae(coords, atom_mask, valid_mask)
			...
			case "-": raise ValueError(f"invalid run_type: {run_type}. must be one of ['inference', 'div_vae', 'bb_vae', 'diffusion']")
	...
	def _run_inference(self, 	coords: torch.Tensor, labels: torch.Tensor, 
		...
		voxel = self._div_vae.dec(latent)

		# predict sequence from decoded voxels
		seq = self._div_vae.classifier(voxel)

		return seq
	
	def _run_div_vae(self, 	coords: torch.Tensor, labels: torch.Tensor, valid_mask: torch.Tensor, atom_mask: torch.Tensor
		...
		_, latent_mu, latent_logvar, decoded_voxels, seq = self._div_vae(voxels)
	...
	def _run_diffusion(self, 	coords: torch.Tensor, labels: torch.Tensor, 
		...
		latent_div, _, _ = self._div_vae.enc(voxels)
		latent_bb, _, _ = self._bb_vae.enc(coords_bb, frames, seq_pos, chain_pos, valid_mask)
