...
class DataHolder():
	...
	def load(self, data_type):
		...
		'''
		if data_type == "train":
			self.train_data = Data(self.data_path, self.train_pdbs, self.num_train, self.batch_tokens, self.max_batch_size, self.min_seq_size, self.max_seq_size,  self.homo_thresh, self.asymmetric_units_only)
		elif data_type == "val":
			self.val_data = Data(self.data_path, self.val_pdbs, self.num_val, self.batch_tokens, self.max_batch_size, self.min_seq_size, self.max_seq_size, self.homo_thresh, self.asymmetric_units_only)
		elif data_type == "test":	
			self.test_data = Data(self.data_path, self.test_pdbs, self.num_test, self.batch_tokens, self.max_batch_size, self.min_seq_size, self.max_seq_size, self.homo_thresh, self.asymmetric_units_only)

class Data():
	def __init__(self, data_path, clusters_df, num_samples=None, batch_tokens=16384, max_batch_size=128, min_seq_size=512, max_seq_size=16384, homo_thresh=0.70, asymmetric_units_only=False, device="cpu"):
	...
	def __iter__(self):
		...
		for batch in self.epoch_biounits.batches: # get the batch indices for this gpu
